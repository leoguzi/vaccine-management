...
class ControladorSistema():
    def __init__(self, tela_sistema: TelaSistema):
        ...
        self.__controlador_agendamento(TelaAgendamento(), self.__controlador_pacientes, self.__controlador_enfermeiros, self.__controlador_vacinas)
    def encerra_sistema(self):
        exit(0)

    def abre_menu_principal(self):
        os.system('cls' if os.name == 'nt' else 'clear')
        lista_opcoes = {1: self.__controlador_enfermeiros.abre_tela_enfermeiros, 2: self.__controlador_pacientes.abre_tela_pacientes, 3: self.__controlador_agendamento.inicia_tela_agendamento, 4: self.__controlador_vacinas.inicia_tela_vacina, 0: self.encerra_sistema} #adicionar as outras opções quando prontas
        while True:
            try:
