        ...
        if lista_pacientes is not None and lista_enfermeiros is not None: #só cria a combobox se existirem pacientes, enfermeiros e vacinas já cadastrados
            ...
            if button == 'Agendar':
                if values['-dados_paciente-'] == '' or values['-dados_enfermeiro-'] == '' or values['-data-'] == '' or values['-hora-']: #clicou em selecionar sem selecionar nenhuma vacina
                    selecionado = {'codigo_paciente': values['-dados_paciente-'], 'codigo_enfermeiro': values['-dados_enfermeiro-'],
                    'data': values['-data-'], 'hora': values['-hora-']}  # retorna '' (string vazia), tratar onde esta função for utilizada.
                else:
                    selecionado = {'codigo_paciente': int(values['-dados_paciente-'].split(' ')[0]), 'codigo_enfermeiro': int(values['-dados_enfermeiro-'].split(' ')[0]),
                    'data': values['-data-'], 'hora': values['-hora-']}#dicionário com o código das informações selecionadas
            ...
            selecionado = None #retorna None caso não existam pacientes ou enfermeiros cadastradas.
    ...
    def seleciona_agendamento(self, lista_agendamentos):
        if lista_agendamentos is not None: #só cria a combobox se existirem vacinas
            ...
            for agendamento in lista_agendamentos:
                cod_paciente = str(agendamento['codigo']) + ' - ' + str(agendament['paciente'])
